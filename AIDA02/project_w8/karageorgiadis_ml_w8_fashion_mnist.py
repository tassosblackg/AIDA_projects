# -*- coding: utf-8 -*-
"""Karageorgiadis_ML_w8_fashion_mnist.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1N6yR0cH-WgglnYl4GaWxAY9J4UAOPtFY
...
Fist I trained a Convolutional Autoencoder model using Fashion_Mnist dataset for **20** *epochs* the model accuracy and loss graphs were smoother and validation with training were very close. Then I used the half-autoencoder model, by taking only the encoder part of the model and stacked it together with a fully connected block (Dense layer) in order to create a classifier. I set *encoder trainable parameters* to *'False'*, and actualy trained only the dense layers of the classifier, for another 20 epochs. The result was an accuracy of ***88.8***% based on test set while the loss was ***0.32***.
...
# New Model
  ...
  Then I took the encoder architecture combined with the fully connected block for classification and trained the new model from start for 40 epochs. The new model converged faster than the previous (9 epochs was enough). As a result I get a higer accuracy of ***91.3%*** and a loss of ***0.29*** based on test data during evaluation. The model took a little more time to be trained per epoch and after *9* epochs started to overfit. 

 The difference between the two methods is not significant so if we have to choose between the two of them we have to take into account if accouracy is important or we need a more generalized purpose model for use in the future.
"""